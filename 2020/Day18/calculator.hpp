...
private:
  ...
  Operator parseOp()
  {
    ...
    switch (getCharacter())
    {
      ...
      case '+': index_++;     return Operator(OPERATOR_ADDITION,       20, 'L');
      case '-': index_++;     return Operator(OPERATOR_SUBTRACTION,    20, 'L');
      case '/': index_++;     return Operator(OPERATOR_DIVISION,       10, 'L');
      case '%': index_++;     return Operator(OPERATOR_MODULO,         10, 'L');
      case '*': index_++; if (getCharacter() != '*')
                              return Operator(OPERATOR_MULTIPLICATION, 10, 'L');
  ...
  T parseExpr()
  {
    stack_.push(OperatorValue(Operator(OPERATOR_NULL, 0, 'L'), (T) 0));
...
template <typename T>
inline T eval(std::string& expression)
{
    ExpressionParser<T> parser;
    return parser.eval(expression);
}
...
/*
inline long int eval(const std::string& expression)
{
  return eval<long int>(expression);
}

inline long int eval(char c)
{
  return eval<long int>(c);
}
*/
inline long long int eval(const std::string& expression)
{
    return eval<long long int>(expression);
}

inline long long int eval(char c)
{
    return eval<long long int>(c);
}
